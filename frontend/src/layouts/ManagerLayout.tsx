...
export function ManagerLayout() {
  ...
  const navLinks = [
    { to: '/manager/orders', label: t.manager.orders, icon: Package },
    { to: '/manager/products', label: t.manager.products, icon: Archive },
    { to: '/manager/categories', label: t.manager.categories, icon: Tags },
    { to: '/manager/inventory', label: t.manager.inventory, icon: LayoutDashboard },
    { to: '/manager/settings', label: t.nav.settings, icon: Settings },
  ];

  const SidebarContent = () => (
    <>
      <div className="p-6 border-b border-slate-100">
        <Link to="/manager/orders" className="flex items-center gap-2.5">
          <div className="w-8 h-8 bg-primary-600 rounded-lg flex items-center justify-center text-white font-bold text-sm shadow-sm">
            M
          </div>
          <div>
            <div className="text-base font-bold text-slate-900">{t.nav.manager}</div>
          </div>
        </Link>
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="md:hidden bg-white border-b border-slate-200/60 px-4 h-14 flex items-center gap-3 sticky top-0 z-40">
          ...
          <span className="text-xs text-slate-400 font-medium">{t.nav.manager}</span>
        </div>
